...
function initializeDataTable(selector, data, unitsKey) {
  ...
  function sortBuildingChartData(sortType) {
    ...
    chartTopBuildingTransaction.data.labels = sortedData.map(
      (property) => property.BUILDING_CLASS_CATEGORY
    ); // Use property.BUILDING_CLASS_CATEGORY as label
    chartTopBuildingTransaction.data.datasets[0].data = sortedData.map(
      (property) => parseFloat(property.SALE_PRICE)
    );
